 ...
 *
 * Drives the store's `isBooting` flag rather than local component state — route guards
 * (ProtectedRoute/PublicOnlyRoute) read `isBooting` directly from the store, so they never
 * have to be told about it via props, and they naturally wait for it regardless of where in
 * the tree they render. Call this once, near the app root (see App.jsx).
 */
export function useAuthBoot() {
  useEffect(() => {
    const { setAccessToken, updateUser, setBooting, logout } = useAuthStore.getState();
    let cancelled = false;
    ...
    authApi
      ...
      .then(({ data }) => {
        ...
        // separately so the header/sidebar have a name and role to show immediately rather
